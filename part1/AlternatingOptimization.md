...
- (A3): $f$ is proper, lower semicontinous.
- (A4): $f$ is convex.
...
|---|---|---|
| {prf:ref}`th:AO`  | (A2,A3,A5,A11)($f$)  | AO limit points are coordinatewise minimum  |
...
- Are the sequences of iterates $\{x^{(i)}_k\}_{k\leq d}$ converging towards a stationary point in the nonconvex case, or the global minimizer in the convex case, and at what rate?
...
```{prf:theorem} Convergence of AO, nonconvex smooth + separable convex nonsmooth 
:label: th:AO2
...
```{prf:theorem} Convergence of AO, convex smooth + separable convex nonsmooth  
:label: th:AO3
...
```{prf:theorem} Convergence of AO, nonconvex smooth 
:label: th:AO4
...
```{prf:theorem} Convergence of AO, $d=2$, nonconvex smooth
:label: th:AO5
...
```{prf:theorem} Convergence of SUM, nonconvex nonsmooth
:label: th:sum
...
%Convergence rates derived later in 2017, but in the convex, nonsmooth case.

%- BSUM CVG rate
%First deposited on arxiv in 2013 !! accepted in 2017
%Similar results as Shefi and Teboulle 2016, convergence rate of PALM.

%BSUM but in the nonsmooth case **BUT convex**, also addresses Block minimization (AO)
...
%- AO can also fail because not all coordinatewise minima are stationary points. Counterexamples in Beck book, even with convex cost. This can be avoided in two-blocks, or with other assumptions as in Grippo Scriandrone.
