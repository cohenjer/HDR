...
```{note}
The following paragraphs deal with radiative transfer, a scientific discpline in optics and physics that I am not very familiar with. In particular, I have not read many scientific articles on this topic, and rather used mediation material such as found on Wikipedia. Here are some links for [radiative transfer](https://en.wikipedia.org/wiki/Radiative_transfer), [color](https://en.wikipedia.org/wiki/Color), and  [additive](https://en.wikipedia.org/wiki/Color) and [substractive](https://en.wikipedia.org/wiki/Subtractive_color) mixtures.
```
